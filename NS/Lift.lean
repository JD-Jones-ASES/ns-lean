...
theorem tail_below (m : ℕ) {p : Param} (hb : Below m p.2.2.2) : Below (m + 2) (tail m p) := by
  unfold tail
...
theorem coeff_tail_self (m : ℕ) {p : Param} (hb : Below m p.2.2.2) :
...
theorem coeff_tail_succ (m : ℕ) {p : Param} (hb : Below m p.2.2.2) :
...
theorem liftMap_injOn (m : ℕ) (B : Finset (ZMod 3)[X]) (hB : AllBelow m B) :
  ...
  intro p hp p' hp' h
  have hpm : p ∈ params B := Finset.mem_coe.mp hp
  have hpm' : p' ∈ params B := Finset.mem_coe.mp hp'
