...
theorem D3_8_ge_810_internal :
  ...
  refine ⟨fam0 1, ?_, ?_, fam0_sdf 1⟩
  · have h : AllBelow 8 (fam0 1) := by simpa using fam0_allBelow 1
    exact (allBelow_iff_degLT 8 (fam0 1) (by norm_num)).mp h
  · simpa using fam0_card 1
...
theorem conjecture13_fails_3_2_8_internal :
  ...
  intro h
  obtain ⟨A, hA, hcard, hS⟩ := D3_8_ge_810_internal
  have hle := h A hA hS
  rw [hcard] at hle
  norm_num at hle
...
theorem D3_8e_ge_internal (e : ℕ) (he : 1 ≤ e) :
    ∃ A : Finset (Polynomial (ZMod 3)), DegLT (8 * e) A ∧ A.card = 810 ^ e ∧ SDF A :=
  ⟨fam0 e, (allBelow_iff_degLT (8 * e) (fam0 e) (by omega)).mp (fam0_allBelow e),
    fam0_card e, fam0_sdf e⟩

/-- **The second family.** For every `e`, a square-difference-free subset of `P_{3,8e+4}` with
`27 · 810^e` elements. -/
theorem D3_8e4_ge_internal (e : ℕ) :
    ∃ A : Finset (Polynomial (ZMod 3)), DegLT (8 * e + 4) A ∧ A.card = 27 * 810 ^ e ∧ SDF A :=
  ⟨fam4 e, (allBelow_iff_degLT (8 * e + 4) (fam4 e) (by omega)).mp (fam4_allBelow e),
    fam4_card e, fam4_sdf e⟩
...
theorem conjecture13_fails_3_2_all_internal (n : ℕ) (h4 : 4 ∣ n) (h8 : 8 ≤ n) :
  ...
  · -- `n = 8e` with `e ≥ 1`: the first family has `810^e` elements against the bound `729^e`.
    ...
    obtain ⟨A, hA, hcard, hS⟩ := D3_8e_ge_internal e he1
  ...
  · -- `n = 8e + 4` with `e ≥ 1`: the second family has `27 · 810^e` against `27 · 729^e`.
    obtain ⟨e, he⟩ := hk
    have he1 : 1 ≤ e := by omega
    obtain ⟨A, hA, hcard, hS⟩ := D3_8e4_ge_internal e
